...
To address the complete problem, we extend the CP model introduced in Section~\ref{sec:first_cp_model} to handle the new constraint. We also formulate a new Mixed Integer Programming (MIP) model and compare the performance of the two formulations on real-world test cases.

Both the CP and MIP models can be solved using appropriate solvers to obtain feasible layout. However, neither approach accounts for fixture rotations, since allowing rotation angles other than multiples of $90^\circ$ significantly increases the number of possible configurations and makes the problem substantially harder to solve.
...
The introduction of rotation at this stage provides additional flexibility in fixture placement, enlarging the set of feasible placements and allowing fixture centroids to be positioned closer to workpiece boundaries or areas to be extruded.
...
Unlike the CP and MIP models, PSO is not affected by the nonlinear operations required to evaluate $\finer$. Consequently, it does not rely on the alternative objective function; instead, it directly incorporates the moments of inertia of the fixture layout into the optimization algorithm.
...
\begin{equation*}
	...
	\begin{cases}
		...
		cx_i + \wfix_{t_i} + \wmin \le cx_j, & \text{if } cx_i \neq cx_j.
	\end{cases}
\end{equation*}
...
Since PSO operates on continuous variable values, during the execution the fixture type values in $P_s[3,j]$ are rounded to the nearest integer to ensure valid fixture types and evaluate the corresponding constraints.

Each position matrix $P_s$ represents a candidate solution associated with particle $s$. The availability of an initial solution obtained from the CP model and the MIP model ensures that the swarm is initialized from feasible layouts.
...
For each workpiece, the best CP and MIP solutions in terms of $\finer$, were used as initial ones for the PSO algorithm. 
...
\begin{table}[!t]
	...
	\caption{Comparison of the best and mean moment of inertia and coefficient of variation (CV) obtained with CP and MIP across repeated PSO runs. CV is the sample standard deviation divided by the mean. Bold values indicate the highest best or mean result for each workpiece.}
	
	\label{tab:pso_cp_mip_comparison}
	
	\begin{tabular}{l c c c c}
		
		\hline
		
		Workpiece & Model & Best & Mean & CV (\%) \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Stair step\\spiral staircase}} & CP & $2.38514\times 10^{9}$ & $2.34680\times 10^{9}$ & 0.936\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$\mathbf{2.38520\times 10^{9}}$ & \cellcolor{gray!15}$\mathbf{2.34714\times 10^{9}}$ & \cellcolor{gray!15}0.945\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Stair step\\simple staircase}} & CP & $6.81046\times 10^{9}$ & $5.26939\times 10^{9}$ & 7.799\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$\mathbf{6.81109\times 10^{9}}$ & \cellcolor{gray!15}$\mathbf{6.81108\times 10^{9}}$ & \cellcolor{gray!15}0.0001\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Dashboard}} & CP & $\mathbf{7.28465\times 10^{9}}$ & $7.28363\times 10^{9}$ & 0.011\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$\mathbf{7.28465\times 10^{9}}$ & \cellcolor{gray!15}$\mathbf{7.28387\times 10^{9}}$ & \cellcolor{gray!15}0.011\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Speaker}} & CP & $\mathbf{3.00956\times 10^{9}}$ & $\mathbf{3.00092\times 10^{9}}$ & 0.641\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$3.00616\times 10^{9}$ & \cellcolor{gray!15}$2.96697\times 10^{9}$ & \cellcolor{gray!15}0.773\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Coffee table}} & CP & $2.27774\times 10^{10}$ & $\mathbf{2.25603\times 10^{10}}$ & 0.594\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$\mathbf{2.27949\times 10^{10}}$ & \cellcolor{gray!15}$2.22810\times 10^{10}$ & \cellcolor{gray!15}1.086\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Door}} & CP & $\mathbf{1.78090\times 10^{11}}$ & $\mathbf{1.75895\times 10^{11}}$ & 1.119\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$1.50433\times 10^{11}$ & \cellcolor{gray!15}$1.37290\times 10^{11}$ & \cellcolor{gray!15}5.093\% \\
		
		\hline
		
		\multirow{2}{*}{\makecell[l]{Door\\with porthole}} & CP & $\mathbf{1.70552\times 10^{11}}$ & $\mathbf{1.70511\times 10^{11}}$ & 0.021\% \\
		& \cellcolor{gray!15}MIP & \cellcolor{gray!15}$1.58425\times 10^{11}$ & \cellcolor{gray!15}$1.51954\times 10^{11}$ & \cellcolor{gray!15}2.823\% \\
...
Table~\ref{tab:pso_cp_mip_comparison} reports the best and average results, together with the coefficient of variation (CV), obtained over 20 PSO runs initialized from the CP and MIP solutions. Overall, the two initialization methods show comparable best-case performance, while CP provides better average results across repeated runs, achieving the highest mean value for four workpieces compared with three for MIP. 

The coefficient of variation further indicates that CP generally provides more consistent results for the more challenging workpieces, particularly for the coffee table and the two door instances. 

\begin{figure}[!b]
	\centering
	\begin{subfigure}{0.7\textwidth}
		\centering
		\includegraphics[width=\textwidth]{figures/comparison_plot1.png}
...
Tables~\ref{tab:visual_results_first} and ~\ref{tab:visual_results_second} report visual comparisons of the best solutions obtained with the CP model and the MIP model for the different workpieces, together with the corresponding solutions refined by PSO. Note that, in the figures, the support bars can be positioned beneath the areas to be extruded, since the fixture height prevents collisions between the cutting tools and the bars.

Rotation plays an important role in the stair step of the spiral staircase and coffee table workpieces, where fixture orientation improves adherence to the workpiece boundary. For the speaker and the dashboard workpiece, the PSO refinement introduce only a slight rotation of the rectangular fixtures, yielding the marginal improvement reported in Figure~\ref{fig:improvement}.

The Stair step of the simple staircase exhibits the largest improvement over the expert layout. This is mainly due to the use of a larger number of fixtures, five for the CP and MIP models, compared with the three fixtures used by the expert operator.
...
\begin{landscape}
	\begin{table}[t]
		...
		\begin{tabular}{c c c c}
			...
			\makecell{Stair step\\spiral staircase} & 
			\makecell{$1.98697\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/spiral_stair_step_expert_operator.png}} & 
			\makecell{MIP: $2.17089\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/spiral_stair_step_mip_model_gurobi.png}} & 
			\makecell{MIP: $2.38520\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/spiral_stair_step_pso_mip_pso.png}} \\
			\hline
			
			\makecell{Stair step\\simple staircase} & 
			\makecell{$4.00911\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/simple_stair_step_expert_operator.png}} & 
			\makecell{MIP: $6.76638\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/simple_stair_step_mip_model_gurobi.png}} & 
			\makecell{MIP: $6.81108\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/simple_stair_step_pso_mip_pso.png}} \\
			\hline
			
			\makecell{Dashboard} & 
			\makecell{$6.29544\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/dashboard_expert_operator.png}} & 
			\makecell{CP: $7.10094\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/dashboard_cp_model_gecode.png}} & 
			\makecell{CP: $7.12882\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/dashboard_pso_cp_pso.png}} \\
			\hline
			
			\makecell{Speaker} & 
			\makecell{$2.69490\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/speaker_expert_operator.png}} & 
			\makecell{CP, MIP: $2.93433\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/speaker_cp_model_gecode.png}} & 
			\makecell{CP: $3.00956\times 10^{9}$ \\ \includegraphics[width=0.18\linewidth]{figures/speaker_pso_cp_pso.png}} \\
			\hline
	...
	\begin{table}[t]
		...
		\begin{tabular}{c c c c}
			...
			\makecell{$2.14371\times 10^{10}$ \\ \includegraphics[width=0.18\linewidth]{figures/coffee_table_expert_operator.png}} & 
			\makecell{CP: $2.21449\times 10^{10}$ \\ \includegraphics[width=0.18\linewidth]{figures/coffee_table_cp_model_chuffed.png}} & 
			\makecell{MIP: $2.27948\times 10^{10}$ \\ \includegraphics[width=0.18\linewidth]{figures/coffee_table_pso_mip_pso.png}} \\
			\hline
			
			\makecell{Door} & 
			\makecell{$1.31109\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_expert_operator.png}} & 
			\makecell{CP: $1.52925\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_LNS_cp_model_gecode.png}} & 
			\makecell{CP: $1.78090\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_pso_cp_pso.png}} \\
			\hline
			
			\makecell{Door\\with porthole} & 
			\makecell{$1.31109\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_porthole_expert_operator.png}} & 
			\makecell{CP: $1.52861\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_porthole_LNS_cp_model_gecode.png}} & 
			\makecell{CP: $1.70552\times 10^{11}$ \\ \includegraphics[width=0.18\linewidth]{figures/door_porthole_pso_cp_pso.png}} \\
			\hline
