...
As discussed in Chapter~\ref{chap:pt1_background}, workpiece stability can be improved by maximizing the principal moments of inertia of the fixture layout. Consequently, the ideal objective function can be defined as:
...
However, computing $J_\xi$ and $J_\eta$ involves nonlinear operations such as squaring and square roots, as well as transcendental functions (sine and cosine) when fixture rotation is considered to determine the transformed fixture coordinates. These operations are difficult to handle efficiently with CP solvers~\cite{leyffer2009applications}.
...
We evaluated our approach on a real case where the workpiece is a stair step of a spiral staircase shown in Figure~\ref{fig:stairstepworkpiece}. The workpiece area, is defined by the following inequalities:
\begin{align*}
...
\begin{figure}[t]
	\centering  \includegraphics[width=0.5\linewidth]{figures/stair_step_line_equations.png}
...
In the tables the optimal solutions found are reported in bold and the $\fdistsemi$ objective function can admit multiple equivalent optimal solutions; however, these solutions may lead to different values of $\finer$, due to the high sensitivity of the moments of inertia to small fixture displacements, as reported for $\nmax \in {3,4,5}$ in Table~\ref{tab:solvercomparison_distobj}.
